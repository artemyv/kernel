...
{ auto res = x; if (!NT_SUCCESS(res)) {\
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "ERROR SysMon.sys: Call failed 0x%x: " #x "\r\n", res)); \
    return res;}}
...
public:
    ...
    NTSTATUS Format(NTSTRSAFE_PWSTR fmt ,ARGS... args)
    {
        ...
        if (!ptr)
        {
            KdPrintEx((DPFLTR_IHVDRIVER_ID,DPFLTR_ERROR_LEVEL,"ERROR SysMon.sys: failed to allocate temp buffer\r\n"));
