...
#include <ntstrsafe.h>
#include <wdm.h>
#define POOLTAG_PPRT 'pprt'

#define RUN_TEST_NTSTATUS(x) \
{ auto res = x; if (!NT_SUCCESS(res)) {\
    KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "ERROR ProcessProtect.sys: Call failed 0x%x: " #x "\r\n", res)); \
    return res;}}
...
public:
    ...
    ~PoolGuard() {
        ExFreePoolWithTag(m_p, POOLTAG_PPRT);
    }
private:
    PoolGuard() = delete;
...
public:
    ...
    ~StringWrapper() {
        if(ptr)
            ExFreePoolWithTag(ptr, POOLTAG_PPRT);
    }
    ...
    NTSTATUS Format(NTSTRSAFE_PWSTR fmt ,ARGS... args)
    {
        ...
        if (!ptr)
        {
            KdPrintEx((DPFLTR_IHVDRIVER_ID,DPFLTR_ERROR_LEVEL,"ERROR ProcessProtect.sys: failed to allocate temp buffer\r\n"));
